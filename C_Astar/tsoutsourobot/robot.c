...
typedef struct Node{
	...
	int pos; //θέση του Node στην ουρα
	struct Node *r_sibling;
	struct Node *child1;
} SearchGraphNode;

int qel=0; //αριθμός στοιχείων ουράς
SearchGraphNode **queue; //ουρα προτεραιότητας ελαχίστου
SearchGraphNode *last_el;//τελευταίο στοιχείο της ουράς
SearchGraphNode *min;

void combine(int i){//Αναδιατασει την ουρά μετα την μεταβολή της
int l,r,mp;         //τιμής μιας απόστασης
...
int q_min(void){//επιστρέφει το πρώτο στοιχείο της ουράς
	if (qel == 0) return -1;
	min = queue[1];
	queue[1] = queue[qel];
	last_el->pos = 1;
...
void insert(SearchGraphNode *el){//προσθέτει ένα στοιχεί οστην ουρά
int i,p;
SearchGraphNode *temp;
...
double distance(int cur_x,int cur_y,int exitx,int exity){//ευκλείδια απόσταση
	double temp;
...
int main(void){
int i,j;
int rob1x,rob1y,rob2x,rob2y;//αρχικές συντεταγμενες των ρομπότ
int x_max,y_max,exit_x,exit_y,newx,newy;
char temp;
int **xy_plane;//0 is O, 1 is X
double **dyn_rob1;//πίνακας για τον δυναμικό προγραμματισμο με -1 να σημαίνει undefined
double **dyn_rob2;//αντιστοιχος πίνακας για το 2ο ρομπότ
SearchGraphNode *root,*root2,*cur_node,*par,*new_node;
int *moves_x,*moves_y; //πινακες του τελικού μονοπατιού του ρομπότ 1
int num_moves = 0;
int temp_move;
int *moves_x2,*moves_y2;//πινακες του τελικού μονοπατιού του ρομπότ 1
int num_moves2 = 0;
...
queue = (SearchGraphNode **) malloc((i+1)*sizeof(SearchGraphNode *)); 
...
//initialisation για αρχική θέση
...
while ((cur_node->x != exit_x) || (cur_node->y != exit_y)){
	...
	for (i=0; i<4; i++){
		...
		if ((newx>0) && (newy>0) && (newx<=x_max) && (newy<=y_max))
			if (xy_plane[newx][newy] != 1){//αν δεν υπάρχει εμπόδιο
				if (cur_node == par){		
				...
				new_node->x = newx;
				new_node->y = newy;
				new_node->state = cur_node->state + 1; //προσδιοριστικό του κάθε κόμβου
				new_node->g = par->g + 1; //η τιμή του g ισούται με αυτή του πατέρα του + 1
				...
				if (dyn_rob1[newx][newy] == -1){//για το σκέλος του δυναμικού προγραμματισμού
					new_node->next = 0; //δεν έχει επεξεργαστεί ποτε ο αλγόριθμός αυτή την θέση
					dyn_rob1[newx][newy] = new_node->g;
					insert(new_node); 			
					}

				else{
					if (dyn_rob1[newx][newy] > new_node->g){//την έχει επισκεφτεί
						new_node->next = 0; //τωρά την επισκεφτηκε με λιγότε βήματα
						dyn_rob1[newx][newy] = new_node->g;
						insert(new_node);}
					else new_node->next = 1; //την εχει επισκεφτεί με λιγότερα βήματα
								 //ο κόμβος αυτός δεν θα επεκταθεί άλλο
				}
	...
	j = q_min();//παιρνουμε στην μεταβλητή min την διεύθυνση του κόμβου της επόμενης κίνησης
	if (j == -1){//αν η ουρά είναι άδεια, δηλαδη η q_min επιστρέψει -1, τότε δεν μπορουμε,
		printf("Robot 1: destination unreachable\n");//να φτάσουμε στο επιθυμητό σημείο
		break;}
	cur_node = min;
	printf("<Robot 1 moving to new position <%d,%d> at step %d\n",cur_node->x,cur_node->y,cur_node->g);
}

if (j!=-1){//αν φτασαμε στο σημείο που θέλαμε
	num_moves = cur_node->g + 1;//εχουμε κανει τόσες κινήσεις
	moves_x = (int *) malloc(num_moves*sizeof(int));
	moves_y = (int *) malloc(num_moves*sizeof(int));
	i=num_moves-1;
	while (cur_node != NULL){//παιρνουμε το μονοπάτι απο το φύλλο ως την ρίζα
		...
		if ((newx>0) && (newy>0) && (newx<=x_max) && (newy<=y_max))
			if (xy_plane[newx][newy] != 1){
				...
				if ((moves_x[temp_move] == newx) && (moves_y[temp_move] == newy)) 
					 if ((newx != exit_x) || (newy != exit_y)){//έλεγχος για σύγκρουση με το άλλο ρομπότ
